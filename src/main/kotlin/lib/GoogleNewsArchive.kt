...
fun googleNewsRequest(
    ...
    val hash = hash(requestUrl)
    val targetFile = File("cache/googlenews_$hash.json")


    if (targetFile.exists() && targetFile.lastModified() > LocalDateTime.now().minusDays(1)
            .toEpochSecond(ZoneOffset.UTC)
    ) {
        logger.info { "using cached results for $requestUrl" }
        val json = targetFile.readText()
        return Json.decodeFromString<Result>(json)
    } else {
        ...
        val json = cache.getOrPut(requestUrl) {
            ...
            con.readTimeout = connectTimeOut
            ...
            logger.info { "received ${json.length} bytes" }
            println(json)
            json
        }
...
fun googleNewsSequence(
    ...
    while (true) {
        for (a in googleNewsSequenceInternal(
            ...
            val fa = FileArticle(
                a.title,
                listOf(a.description ?: " "),
                listOfNotNull(a.urlToImage),
                a.publishedAt,
                a.source.name,
                a.author
            )
